...
/**
 * Manage shared volumes: loop-image ext4 filesystems mounted into attached
 * agents at ~/Shared/<name>. The fixed filesystem size is the hard cap —
 * agents writing past it get ENOSPC. Attach/detach lives in each agent's
 * settings (and in the create-agent form); this modal is create/inspect/delete.
 */
export function VolumesModal({
  isOpen,
  onOpenChange,
}: {
  isOpen: boolean;
      ...
      <Modal.Backdrop isOpen={isOpen} onOpenChange={onOpenChange} isDismissable={!busy}>
        <Modal.Container placement="center">
          <Modal.Dialog className="sm:max-w-[560px]">
            <Modal.CloseTrigger />
            <Modal.Header>
              <Modal.Heading>Shared volumes</Modal.Heading>
            </Modal.Header>
            <Modal.Body>
              <p className="text-muted mb-3 text-sm">
                A volume mounts into every attached agent at{' '}
                <code className="text-xs">~/Shared/&lt;name&gt;</code> — same files, hard size cap.
                Attach from an agent&apos;s settings or when creating one; attach/detach applies on
                the agent&apos;s next rebuild.
              </p>

              {/* Create form */}
              <div className="border-separator mb-4 flex flex-wrap items-end gap-2 border p-3">
                <TextField
              ...
              {vols && vols.length > 0 && (
                <ul className="border-separator divide-separator divide-y border">
                  {vols.map((v) => {
                    const pct =
                      v.usedMb !== null
                        ? Math.min(100, Math.round((v.usedMb / v.sizeMb) * 100))
                        : 0;
                    return (
                            ...
                            <Tooltip>
                              ...
                              <Tooltip.Content showArrow className="max-w-[280px]">
                                ...
                                <p className="px-1 py-1.5 text-xs">
                                  The loop filesystem isn&apos;t mounted (or hasn&apos;t propagated
                                  yet). The gateway re-mounts volumes at boot; if this persists,
                                  restart the dashboard stack.
                                </p>
                              </Tooltip.Content>
                            </Tooltip>
                          )}
