...
export const CAPABILITIES: {
  ...
  },
  {
    ...
    mcpHelp:
      "Tool: swarm_set_effort(effort). `effort` = one of low, medium, high, xhigh, max, ultracode, or default (clears the override). The gateway drives Claude Code's `/effort <level>` slash command in your own TUI — Esc to interrupt whatever is in flight, then an Enter to clear the line, the command, then confirmation Enters spaced over ~11s while the selector redraws — so expect keystrokes to appear in your session; that's the switch, not interference. The Esc will cut your current turn short, which is expected. A **[sys://effort]** message confirms when it's done. The level is also persisted to your identity, so it survives a session respawn instead of silently reverting. Scope: self only — you can never change a peer's effort. Ultracode means max effort plus multi-agent Workflow orchestration: it is substantially slower and burns far more tokens, and it wants headroom beyond the default 2-core / 4 GB agent, so switch to it deliberately for genuinely hard work and drop back down afterwards.",
  },
  {
    key: 'restart_self',
    label: 'Restart self',
